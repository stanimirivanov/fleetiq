...
public class DeviceProjectionOutboxRelay {
    ...
    @Inject
    public DeviceProjectionOutboxRelay(PgPool pgPool,
        @Channel("device-projections-out") MutinyEmitter<byte[]> emitter) {
        this(pgPool, emitter::send);
    }

    DeviceProjectionOutboxRelay(PgPool pgPool, Function<byte[], Uni<Void>> publisher) {
        this.pgPool = pgPool;
