...
@QuarkusTest
class DeviceProjectionOutboxRelayIT {
    ...
    void competingRelaysPublishEachRecordOnce(UniAsserter asserter) {
        ...
        var first = new DeviceProjectionOutboxRelay(pgPool,
            payload -> Uni.createFrom().item(() -> { published.add((int) payload[0]); return null; }));
        var second = new DeviceProjectionOutboxRelay(pgPool,
            payload -> Uni.createFrom().item(() -> { published.add((int) payload[0]); return null; }));
        ...
        asserter.assertThat(() -> countRows(), count -> {
    ...
    void failedPublicationRollsBackDeletion(UniAsserter asserter) {
        var relay = new DeviceProjectionOutboxRelay(pgPool,
